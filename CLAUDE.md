...
| 雷达打到自车车顶 | 34% 的点落在自己车顶上，感知会当成零距离障碍物 | （2026-08-03 数据，雷达 z=1.6、车顶 1.5 只高 10 cm。）**抬高雷达没用**（车顶长 4.4 m）—— 按自车轮廓裁剪（`lidar_preprocessor` ego_box）+ `range_min` 近裁剪（下面那条）。⚠️ P9-S3b 起雷达抬到 **2.2**（Δz=0.7），动机在 CARLA（无近裁剪，c3 车顶 1.571 → 60% 自反射），Gazebo 这两道防线照旧 |
...
| **判据只有外界没有内界** | 扩窗实测：自车过弯贴行人 2.2–2.8 m 经过，连续 11 帧"漏检"——按判据算 FAIL，而那是 `range_min=2.2` 的**近场盲区**（b54345c 修复时文档写明的代价） | 与「>30 m 物理上不可检」同理：判据范围两头都要由物理定。现内界 3 m。同批教训：符号判据要过跟踪器自己的 `heading_min_speed` 门限（刚建的航迹速度是噪声）；ID 连续性只在中断 ≤ 删除+滑行设计窗口（3.5 s）内判——**系统设计上保不住的中断换 ID 是正确行为不是缺陷** |
...
| **CARLA 传感器回调里 `np.frombuffer(data.raw_data)` 不立刻 `.copy()`** | 拼帧后每帧 = **同一半圈 × 2**（每通道 1800 点全挤在 [0°,180°)、另一半圈空空如也），另半个世界从没进过点云：NPC 车 3.5–35 m 全程 0 点、行人 10 m 外 0 点、检测率表 0%；而分割/聚类/跟踪每一层看起来都在「正常工作」 | P9 窗口 4（2026-08-15）铁案：`raw_data` 是 `PyMemoryView_FromMemory` 直接套在 libcarla 测量对象内存上、**不持有所有权**，回调返回后内存复用给下一拍；存起来等第二拍再拼的第一块 chunk 于是「变成」第二拍的内容。此前 `.copy()` 放在 concatenate 之后对第一块无效。窗口 3 在这份坏点云上落了十刀感知调参（含「不翻 y 铁案」），每一刀都「有效果」。**云上任何感知判据变红，先跑 `scripts/p9_lidar_probe.py`（裸测：raw 帧方位角直方图是不是整圈、真值邻域点数、离地点质心偏差）再碰感知参数** |
| **同步模式下新客户端第一次 `get_actors()` 为空** | 「栈起 40 s 后 npc actor 从 get_actors 消失」—— 追凶一夜、瞬移隐形理论作废又立、仪器写了两套 | actor 列表来自本客户端**最近收到的 episode state**，`GetCurrentEpisode` 在同步模式**不等 tick 就返回**：连上即查 → frame 0 / elapsed 0.0 / 0 个 actor；1.5 s 后再查 → 7 个。仪器（sidecar `_npc_census` + 独立 `p9_actor_watch.py`）实测 actor 全程在、真车物理闭环跑完全程。查 actor 先睡一拍，每行印仿真钟 —— **仿真钟不走 = 视野过期，那行不算数** |
| **判点云 y 符号的实验输入只有半边世界** | 「正窗 0 / 镜像窗 58054」被写成 y 不用翻的铁案，sidecar 去掉了翻号；栈上跑出来的检测率 0% 才把它逼出来 | 那份帧里只有一半世界（上一条），「0 对 58054」量的是「哪半边世界在帧里」不是符号。裸测把真值算到传感器系（车在 y=−3.5）时 raw 点也在 y=−3.5 ⟹ **raw 是 UE 左手系 y 右为正，转 ROS 要翻**。判符号：先确认 raw 方位角直方图整圈；只数**离地点**（地面对镜像无差别，60 帧几万地面点把车那几百点淹没成 INCONCLUSIVE）。`p9_mirror_probe.py` 现在按此实现 |
| **把 CARLA actor 原点当 base_link 发** | 整片点云沿车头前移 1.41 m：行人近边误差恒 +1.1～1.3 m、车过身时横向误差 0.78 —— 全都像感知的账；控制的前轴换算也偏 1.41 | CARLA 车辆 actor 原点 = 包围盒 **xy 中心**，SPEC/URDF 的 base_link 是**后轴**。原点→后轴偏移从 `get_physics_control().wheels[2:4]` 读（c3 −1.410；⚠️ 第一拍物理之前轮位是垃圾，实测 −28.6，先 tick 两拍），不写常数。传感器安装也按它换算（此前按我们盒子车的 1.35 近似）。walker 原点在身体中心（extent z 0.93，落定 root z 0.951），瞬移到 z=0.2 = 埋进路面 0.75 m → 雷达只见上半身 0.6 m 高 → STATIC；真值 z 要报**物理底面**，否则记录器的离地校验永远看不见埋地（它一直读的是脚本 z=0） |
| **生成世界的护栏墙在弯角** | 感知层：车在东南角前 20 m「自己停了」，353 帧车道内虚警全是 52×15 m（h1.0）与 6×6 的框，真值层全绿 | 墙在弯角是**直墙 + 圆弧墙的 L 形连通簇**，OBB 是空腹大框、中心落在车道里 → 结构物航迹照实发布 → 规划当拦路静物。OBB 描述不了凹形大结构（perception.md 边界节）；试 additional_width=25 让墙退出 30 m 量程 —— 内圈弯角半径 12−1.75−25<0 网格自交、墙折回路内，更糟。定案 `wall_height=0`（Gazebo 同处本来是草地，两环境感知输入同构）；6 m 裙板留作第一道容错，出界 6 m 以上会坠虚空让引擎崩（P8-S6 老账，行为判据全在路内） |
| **「静态标定更准」≠「闭环更好」（CARLA 油门映射，两次应验）** | P8：五点精标 `0.479+0.142·a` 静态更准、闭环稳态 0.42→0.57 还卡墙；P9-S5d：拉平传动系后静态图变直（各速段增益 4.5–4.7），闭环 S01 只赢 Δv（0.42→0.25），车停在目标前 3.96 m | 油门→加速度是 (throttle, v) 二维现象（PhysX 零油门阻尼 2.0 vs 全油门 0.15 + 扭矩曲线峰在 1 挡 4.8 m/s，`scripts/carla_throttle_map_probe.py` 二维表），而闭环还叠着蠕动偏置 / 制动-滑行配平 / 20 Hz PI 三件事 —— 改一处静态映射就跑闭环判据，别只看静态残差。S5d 拍板不根治，机制留档 `ego.align_drivetrain`（默认关），见 ADR-0002 |
| **道具几何不贴 ODD、车队共用首航点在物理化的 CARLA 里** | ① CP-P9-A 差一项：近边 p95 0.60（判据 0.5）恒偏近，车 15–20 m p50 0.52；② junction 真值层「让行 False」、对车流最近 15.8 m —— 路口根本没有车流 | ① 感知的车辆先验把框长垫到 ODD 车长 4.4，道具 micra 3.63 比它短，框往两头各多出 (4.4−3.4)/2 ≈ 0.5 —— 是**刺激物几何**不是感知；换成 33 个蓝图里最贴 4.4×1.8×1.5 的 seat.leon 后 9/9。② 三辆 cross_car 共用首航点，CARLA spawn 碰撞检查只放第一辆上路，P9-S2 真车化（重力开、不再瞬移）后回退高空的 b/c 直接摔在 a 身上。修为「天上停车场」：关重力挂着等出发相位，npc_controller 第一条非零 cmd 时落到首航点（前车已走 6 s）。**改 NPC 驱动方式（瞬移 → 物理）之后，所有依赖「道具可以重叠」的场景约定都要重审** |
...
真身是「补全/重锚虫洞焊出假速度航迹 × 遮挡滑行」，见 perception.md §6.6）。
**P9-S5c（2026-08-16）加了两道物理先验准入门与一个成熟门槛**（perception.md §6.2b/§6.7/§7）：
...
# 租机窗口开场一键（宿主侧，幂等，五个里程碑）：scripts/cloud_window_open.sh（用法在脚本头）
# P9 传感器裸测（云机容器内，**栈没在跑时**）：raw 帧方位角直方图是不是整圈 / 自反射
#   占比 / 真值邻域点数 / 离地点质心偏差 —— 云上感知判据变红先跑它，再碰感知参数：
...
#   http://localhost:8080（追尾）/top（跟车俯视 35 m）/map（全图）—— 用法在 docs/p8_carla_bringup.md §7
```
