...
| 6 | 预测断流 / 从未到达 | 规划 `prediction_timeout_s` / `expect_prediction` 同 4/5 | 代码路径与 4/5 同一段（planning_node 376–403 行）；**无专门注入** | ⚠️ 缺口（低风险：与 4 同一段代码，同一个 dt 逻辑） |
| 7 | 点云陈旧（stamp 落后 >0.15 s） | 感知丢弃并计数 `dropped_stale_clouds` | perception_node `max_cloud_age_s`；无注入用例 | ⚠️ 缺口 |
...
| 9 | 控制静默 / 控制节点崩 | bridge 看门狗（0.5 s 无指令 ⟹ 全刹） | Gazebo：`verify_teleop.sh`（6.3 s 后 8.33→0，要 GPU）；CARLA：sidecar `_apply_control` expired ⟹ `full_brake`（每 tick 重发） | ✅ 手工脚本 / ⚠️ 无 CI 用例 |
...
| 11 | 定位失锁 / 雷达断流 | NDT `max_innovation` 安全阀；连续 ≥10 帧超门强制粗网格重锚；NDT 1.0 s 无成功帧降出 NDT_AIDED；GNSS 2.0 s 超时 | L3-G 定位闭环 ③c（`ndt_chi2_exceed` 计数）+ 注入实测 32 次重锚；超时降级**无注入用例** | ✅ / ⚠️ 超时分支缺口 |
| 12 | 真值道具飞天 / 埋地 / 倾倒 | 记录器判「本次运行无效」拒绝打分 | `record_perception_run.py` 刺激物校验（现在读**物理** z）；`p9_lidar_probe` 落定高度 | ✅（评测层） |
| 13 | TF 缺失（`map→base_link` 从未到达） | 感知跳帧 warn；规划不发；控制 NO_PATH | 静态 TF 永不过期 ⟹ 只能「一开始就不发」测（陷阱表）；L3-G 定位闭环机械查 `/tf_static` 上不许有 `map→odom` | ⚠️ 「一开始就不发」无专门用例 |
...
| 15 | 目标点无路由 | map_node 报错，控制 NO_PATH 停车 | `test_routing`（L1 无路可达）；L3-G 无 | ⚠️ |

**读法**：✅ 有自动化红绿；⚠️ 代码路径在、无专门注入（写明理由或列为缺口）；❌ 已知未处理。

## 2. 补缺顺序（按风险 × 成本）

1. ~~#14 `/clock` 停走~~ ✅ 已补（2026-08-16，桥侧墙钟守卫 + L3-G）。
2. #7 点云陈旧、#6 预测断流：一个用例各加一段注入即可（复用 test_perception_silence /
   test_closed_loop_perception 的骨架）。
3. #11 定位超时降级、#13 TF 一开始就不发、#15 无路由：各一条 L3-G。
4. #9 控制静默的 CI 用例：test_vehicle_cmd_bridge_watchdog 加「停发」一段。
