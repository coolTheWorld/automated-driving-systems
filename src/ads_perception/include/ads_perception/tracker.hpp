...
//     而先验又依赖分类、分类又依赖尺寸 —— 循环。P8-S2b 用**运动**打破了循环
//     （速度 ≥ 2.5 且正对/背对且宽 ≥ 1.4 的只有车），P9-S5c 把先验改成沿
//     **车头方向**补成 4.4 × 1.8 的先验盒（见 vehicle_prior_width_m 的注释）。
//
//  ## ⚠️ 只信**成熟**航迹的速度（P9-S5c，Gazebo bag 逐帧钉死的三个鬼影同一个根）
//
//  恒速 KF 的速度要十来帧才收敛（σ_v ≈ 0.3·√(12/n³)/Δt：3 帧 2 m/s、10 帧 0.33）。
//  刚凑够 3 帧确认的航迹速度还是噪声，而三处规则都在**信任速度**：
//    · 重复合并的速度一致性判据 —— 年轻重复并不掉，与正主并存 1–2 帧（ID 切换 +2）；
//    · 遮挡滑行 —— 按噪声速度外推 3 s 就是 6 m 的鬼影（车身碎片滑进自车车道）；
//    · 未命中帧的发布 —— 发的是按噪声速度外推的预测位置（同一只碎片，3 帧虚警）。
//  三处统一由 `mature_hits`（10）把关：年轻航迹只按距离并、不滑行、未命中帧钉在
//  最近观测位置发（Track::published_position）。
//
//  ## 180° 二义性在这里消歧
//
...
struct TrackerParams
{
  ...
  /// 车辆形状先验：补全用的车宽，m（= ODD 车宽，vehicle_params 同源）。
  ///
  /// P9-S5c 把先验从「沿视线推 (4.4 − 深度)/2」改成「沿**车头方向**把观测框补到
  /// 4.4 × 1.8 的先验盒」（`AnchorOffset` 同一套几何，缺口补在背离传感器的一侧）：
  ///   · 沿视线推在斜视（邻车道 10–20°）时横向偏 0.35–0.7 m —— 中心不在车上；
  ///   · 旧版只在观测深度 < 1.0 时推，侧面刚露出来（深度 1.0 → 1.8）那一帧推量
  ///     从 1.7 m 跳到 0 —— 新息一跳 1.3 m 判不进门，**每次由远及近必换一次 ID**
  ///     （Gazebo 实测每轮 t≈14 s、18–19 m 处稳定切换一次；bag 回放钉死）。
  ///   按车头方向算「沿/横」两个方向的观测延展、各补到先验尺寸，推量随侧面
  ///   露出连续收缩到 0，中心全程落在车上。
  /// 横向补全的意义：只见侧面（横穿的车）时观测横向延展 0.3，中心该在 0.9 m 外
  /// 那一侧 —— 与长度那一半是同一件事。
  double vehicle_prior_width_m{1.8};
  ...
  /// 航迹的**成熟门槛**（累计命中帧数），P9-S5c：两处规则只信成熟航迹的速度。
  ///
  /// 恒速 KF 的速度要十来帧才收敛（σ_v ≈ 0.3·√(12/n³)/Δt：n=3 → 2 m/s，
  /// n=5 → 0.9，n=10 → 0.33）。刚凑够 3 帧确认的航迹速度还是噪声，而两条规则
  /// 都在**信任速度**：
  ///   · 重复合并的速度一致性判据（merge_speed_mps）—— 年轻航迹速度差正主 2 m/s
  ///     量级、按 1.0 判必不合并，车身碎片起的年轻航迹与正主并存 1–2 帧，评测在
  ///     两者间摆，每次记 2 次 ID 切换（Gazebo 实测 12.8/53.5 s，间距 0.15–0.5 m）；
  ///     年轻的那条只按距离并：ODD 里两个目标中心最近 1.75 m，1.0 m 内的年轻航迹只能是重复；
  ///   · 遮挡滑行 —— 滑行是按速度**外推**最多 3 s，3 帧的速度外推 3 s 就是 6 m 的
  ///     鬼影（Gazebo 实测：车身碎片 3 帧确认后躲在车框后按 5.5 m/s 的假速度滑进
  ///     自车车道，虚警 3 帧）；年轻航迹按普通未命中倒计时。
  /// 取 10（1 s）：速度已收敛到合并门限的三分之一，3 s 外推的位置不确定度 ~1 m。
  /// 调小到 3 → 退化回「已确认就信速度」，上面两个症状回来；
  /// 调大到 30 → 刚进视野 3 s 内被遮挡的目标不滑行、换 ID；擦身而过的两个目标里
  /// 年轻的那条会被并掉。
  int mature_hits{10};
...
struct Track
{
  ...
  /// 出生以来经历的帧数（每次 Update 加一，出生那帧为 0）。
  int age_frames{0};
  /// 出生那一帧，离本检测 ≤ anchor_shift_max 处正有一条**已确认且这一帧丢失**的
  /// 航迹 —— 记下它的 id：本航迹很可能是那条航迹换了个框（重锚），而不是新目标。
  /// 0 = 没有这样的航迹。只在本航迹**确认**那一帧兑现一次（见 Tracker::Update）。
  std::uint32_t reanchor_of_id{0};
  /// 已被自己的重锚航迹取代：**不再享受遮挡滑行**，按普通未命中倒计时；
  /// 再次命中即清除。见 tracker.cpp 里 U 转鬼影那段的推导。
  bool superseded{false};

  /// 最近一次**观测更新后**的位置（ApplyUpdate 写）。发布年轻航迹的未命中帧时用它。
  ...
  /// 连丢 3 帧，预测位置按 5 m/s 的假速度滑进自车车道，车道内虚警 3 帧）。
  /// 钉住而不是不发：年轻的正主丢一两帧也要保持在发布里，否则评测/下游配到旁边
...
public:
  ...
  /// ⚠️ 年轻航迹（hits < mature_hits）的**未命中帧**照发但不外推（P9-S5c）：发布出口
  /// 用 Track::published_position()。
  /// ⚠️ **按值返回**：拿指针/引用之前先落成本地变量，绑在临时量上就是悬空
  ///    （test_tracker.cpp 里踩过，printf 出对的数只是运气）。
...
private:
  ...
  /// 新航迹刚确认时兑现"重锚候选"：把它取代的那条旧航迹标成 superseded（不再滑行）。
  void SupersedeReanchoredTrack(const Track & newborn);
