...
**当前位置**：**P3 验收通过 —— CP-P3-B 三个场景全部达成（2026-08-04，真 Gazebo）。
`safety.margin_m` 维持 0.5，有实测依据。P3 可以收尾，但有两个记在案的问题（见下）。**
...
| 规划器报告不可行的拍数 | **441** | > 0 |
| 终速 | **0.000 m/s** | < 0.10 |
| 停车点到障碍物间距 | **4.265 m** | ≥ 0.50 |

### 场景三「回归」（无障碍物，CP-P2-B 全量）8/8

全部通过。命令与判据见 `record_control_run.py`。
...
## ⚠️ 两个实测出来的问题（判据都过了，但记在案）

### ① 速度超调吃掉了 CP-P2-B 的大半余量
...
根因**不在横向**：全程 10% 的拍数速度超目标 > 0.5 m/s，最大超 **2.13 m/s**，
最高车速 **6.53**（巡航 5.556）。机制是算术，日志逐拍对得上：

```
a_cmd = a_ff + K_p·(v_ref − v) = 1.50 + 1.0×(5.17 − 6.38) = +0.29
```

`a_ff` 上限 = `max_accel` 1.5，`K_p` = 1.0 ⟹ **车速要超目标 1.5 m/s 指令才变负**。
横向劣化跟着它走（`a_lat = v²κ`，速度高 17% ⟹ 横向加速度高 37%）。

P2 没有这个问题，是因为那时剖面覆盖整条路线、只算一次；现在是 30 m 滚动窗口、
每 100 ms 重算，**窗口首点的速度是该点的曲率限速，它不知道车实际多快**。

三个候选方向见 `docs/modules/planning.md` §7。**没在本轮修**：它跨模块
（剖面在规划、增益在控制），且判据仍全过 —— 按规矩在检查点汇报、不自行决定。

### ② 停车点早一个轴距

场景二实测停车间距 **4.265 m**，设计值 1.5 m。差值 ≈ **轴距 2.7 m**：

```
实测 4.265 = 设计 1.5 + 轴距 2.7 + 量化 0.065
```

**规划器按后轴排轨迹，控制器按前轴判到达。** 停车轨迹末点是"后轴该停在哪"，
而 `control_node` 的 `goal_distance_m` 量的是**前轴**到末点的距离。
方向保守（早停），不紧急；但**配置里的 `stop_margin_m` 不等于实际留的距离**。

---

## 🔍 这一轮踩到的两次「判据错了，不是系统错了」

两次都表现为 FAIL，而系统其实是对的 —— 现场长得一模一样：
