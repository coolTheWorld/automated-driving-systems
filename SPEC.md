...
| **控制 ads_control** | 横向 Stanley，纵向双闭环 PID（速度环+加速度环） | LQR、MPC |
...
#### CARLA 侧的接入方式：原生 ROS 2 + 自研 sidecar（双通道）

CARLA 0.9.16 起，**服务端内置 ROS 2 支持**（`--ros2`，内嵌 Fast DDS），
不再需要 `carla-ros-bridge`。这很重要——官方 ros-bridge 仓库停在 0.9.13，
**对新版 CARLA 实际上已无人维护**，继续用它是给自己挖坑。

但原生接口目前是**半成品**，只是一根 "sensor-out / control-in" 的管子：

| | 原生 ROS 2 提供 | 原生 ROS 2 **缺失** |
|---|---|---|
| 输出 | 相机、激光雷达、语义雷达、毫米波、IMU、GNSS、碰撞、压线事件；`/clock`；各传感器 transform | **完整 TF 树**、**ego 车辆状态与静态信息**、**红绿灯**、**地图话题** |
| 输入 | `vehicle_control_cmd` | — |

**我们的方案是双通道，各取所长**：

```
高频重载数据（点云、图像、IMU）  ──原生 ROS 2 / Fast DDS──▶  算法栈
低频轻量数据（车辆状态、红绿灯、  ──PythonAPI──▶ carla_bridge ──▶  算法栈
              地图、完整 TF 树）              (sidecar 节点)
```

- **传感器流走原生通道**：这是带宽大头，原生接口延迟低、无中间进程拷贝
- **状态/地图/TF 走 PythonAPI sidecar**：这些是低频小数据，用 Python 完全够用；
  CARLA 0.9.16 提供 **Python 3.12 官方 wheel**，Ubuntu 24.04 直接可用

> **为什么这个缺陷对我们杀伤力有限**：SPEC §4.1 末尾早已要求
> `ads_simulation` 做成适配层——原生接口缺的 TF 树和 ego 状态，
> **恰好就是适配层本来就要产出的东西**。别人拿原生接口直接插 Autoware 会卡住，
> 我们本来就要写这一层。

**P0b 必须实测确认的已知问题**（均为 CARLA 仓库未关闭 issue）：

1. 车辆控制在某些情况下失效
2. 话题名出现双斜杠
3. 原生接口与 PythonAPI 之间存在坐标系不一致
4. 内嵌 Fast DDS 版本与 Jazzy 自带版本的互操作性
...
| CARLA `--ros2` 与 Jazzy 互通性 | P0b | 单开 Humble 容器只跑 `carla_bridge` |
| CARLA 原生 ROS 2 的三个已知 bug | P0b | 用 PythonAPI sidecar 绕过 |
...
| **P9** | A+B | 性能优化与鲁棒性 | 各模块耗时达标、异常处理完备 |
