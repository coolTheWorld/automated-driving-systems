...
std::vector<Pose2D> MakeStraightAlongX(double length_m, double y_m, double spacing_m)
{
  ...
  for (int i = 0; i <= segments; ++i) {
    // **等分**而不是"起点 + i×步长"：后者在 length/spacing 恰好是整数时，
    // 浮点上可能多算一步，导致最后两个点重合。P1 踩过一次（见 CLAUDE.md 陷阱表），
    // 症状是 RViz 里完全看不出来，而下游按弧长参数化时除以零。
...
/// 多圈的 R=8 左转圆，圆心在原点，起点在 (R, 0) 朝 +y。
/// 圆弧闭环用例要跑十几秒，一圈只有 50 m，不给足圈数车会开出路径末端。
std::vector<Pose2D> MakeCircleLaps(double radius_m, double laps, double spacing_m)
{
  return MakeLeftArc(radius_m, 0.0, 0.0, 0.0, 2.0 * M_PI * laps, spacing_m, false);
}

/// 直路 → R=8 左转弯：曲率在一点之内从 0 跳到 1/8，是控制器能遇到的**最硬**的输入。
std::vector<Pose2D> MakeStraightThenLeftArc(
  double straight_length_m, double radius_m, double sweep_rad, double spacing_m)
{
  std::vector<Pose2D> poses = MakeStraightAlongX(straight_length_m, 0.0, spacing_m);
  // 圆心在直路末端的正左方（左转），起始参数角 −π/2 对应切向 0（= 直路方向）。
  const std::vector<Pose2D> arc =
