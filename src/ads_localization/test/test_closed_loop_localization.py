...
class TestLocalizationClosedLoop(unittest.TestCase):
    ...
    def test_localization_wiring(self):
        ...
        # ---- ③c 严格卡方监视器不许在同源世界里报数（P8-S2c）----
        # 本世界与地图同源、误差毫米级，d² 距门限（16.27）有两个量级余量，
        # 计数器必须为 0。**监视器被误配（门限/地板/膨胀系数错了）时闭环
        # 判据照样全绿** —— 注入实测：门缩到 0.01 后好帧全部超门，硬拒版
        # 靠恢复帧续命、监视版根本不动数据流，两版其余判据都不红 ——
        # 唯一可观测症状就是这个计数器，所以机械断言它。
        # （历史：这道门做过硬拒/降权/步长封顶三版逐帧拦截，真世界实测
        #   全部劣化精度 —— 0.5 m 尺度上好帧与坏帧没有空档，见
        #   localization.md §10.6b。现在只计数 + 连跳 ≥10 帧强制重锚仲裁。）
        chi2_exceed = int(float(self.diagnostics[-1]['ndt_chi2_exceed']))
        self.assertEqual(
            chi2_exceed, 0,
            f'严格卡方监视器计了 {chi2_exceed} 帧 —— 本世界误差毫米级不该触发，'
            '要么门/地板被误配，要么真出了锁错（标定见 localization.md §10.6b）')
