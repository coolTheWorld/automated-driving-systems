  ...
  double target_speed_mps, double feedforward_accel_mps2, double measured_speed_mps, double dt_s)
{
  ...
  // ---------------------------------------------------------------------------
  //  制动前馈按实际车速衰减（P8-S2d，junction 实测逼出）
  // ---------------------------------------------------------------------------
  // 前馈的物理量是 a = v·dv/ds（**实际** v，control.md §7 的公式），而剖面
  // 点上存的是 v_ref·dv/ds —— 两者在车跟得上剖面时相同，**车停在别人的
  // 制动带里时天差地别**：让行后停在离轨迹末端 0.8 m 处重启，剖面前馈
  // −3.0 + P 项 +2.19 = 恒 −0.81 —— 目标速度 2.19 却**永远起不了步**，
  // 车就此在离 goal 1.15 m 处冻住（TRACKING、指令流全正常，没有任何报错）。
  //
  // 按 v/v_ref 缩放即恢复真实物理。两个刻意的收窄，都是为了不动已验收的行为：
  //   · 只作用于**负**前馈（制动带）——加速斜坡的前馈行为保持原样；
  //   · 比值夹在 [0,1]（只衰减不放大）——超速进制动带仍由 P 项处理，与现状一致。
  // v_ref → 0 的端点不在这里处理：goal_reached 分支要的恰恰是「不随速度
  // 衰减的承诺」（-max_decel），它不走本函数的前馈参数。
  if (feedforward_accel_mps2 < 0.0 && target_speed_mps > 1e-6) {
    const double ratio = std::clamp(std::max(measured_speed_mps, 0.0) / target_speed_mps, 0.0, 1.0);
    feedforward_accel_mps2 *= ratio;
  }
