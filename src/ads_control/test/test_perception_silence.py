...
"""
Perception-silence chain: a dead perception must stop the car, through planning.

P9-S5b 异常注入清单里第一条**跨模块**的链路故障（此前的三条守卫各守一段：
odom NaN → 控制不死；规划静默 → 控制刹停；cmd NaN 流 → bridge 刹停）。
这一条把两段接起来：**感知/真值链路中途断流** ⟹ 规划器的
`safety.obstacle_timeout_s`（1.0 s）判「障碍物列表没有更新」、本周期不发轨迹
⟹ 控制器 `safety.trajectory_timeout_s`（0.5 s）判轨迹静默 ⟹ TRAJECTORY_STALE 刹停。

⚠️ 「链路存在但断了」与「链路本来就不存在」是两回事（planning_node 的注释）：
   CP-P2-B 基线整个话题没有发布者也要照常规划。所以本测试**先发**一段 10 Hz 的
   （空的）/perception/obstacles 让链路"存在"，车跑起来之后**戛然而止**。
   `expect_perception=true` 也一并传进去 —— 那是"从未到达"那半边的守卫，
   与这里的"到达过又断了"互补，两条都在这一次运行里被走到。

链路：假车（/clock 来源）+ map→odom + map_node + planning_node + control_node；
障碍物流由测试自己发。判据用**诊断消息的仿真时间戳**量延迟，不用墙钟
（3 倍速下墙钟数没有意义，与 test_trajectory_timeout 同一条纪律）。

## 故障注入实测（2026-08-16，写完立刻做的）

| 注入 | 结果 |
|---|---|
| planning `safety.obstacle_timeout_s` 改成 1e9（复原"没有过期检查"） | **红**：静默后规划器
  拿着最后一份障碍物列表继续规划，车一路 TRACKING 到 GOAL_REACHED，从未 STALE |
"""
...
# 与 planning_params.yaml 的 safety.obstacle_timeout_s、control_params.yaml 的
# safety.trajectory_timeout_s 同步。写死而不读 YAML，理由同 test_trajectory_timeout。
OBSTACLE_TIMEOUT_S = 1.0
TRAJECTORY_TIMEOUT_S = 0.5


@pytest.mark.launch_test
def generate_test_description():
    """Launch fake vehicle + TF + map + planning(expect_perception) + control."""
    fake_vehicle = os.environ['FAKE_VEHICLE_EXECUTABLE']
    planning = planning_params()
    planning['expect_perception'] = True
...
class TestPerceptionSilenceStopsTheCar(unittest.TestCase):
    """Feed an (empty) obstacle stream, go silent, demand planning ERROR + control stop."""
    ...
    def setUp(self):
        self.node = RclpyNode('perception_silence_tester')
    ...
    def _spin(self, wall_seconds, publish_obstacles):
        """Spin for wall_seconds; if publish_obstacles, keep an empty 10 Hz stream up."""
        deadline = time.monotonic() + wall_seconds
        next_pub = 0.0
        while time.monotonic() < deadline:
            if publish_obstacles and time.monotonic() >= next_pub:
                msg = ObstacleArray()
    ...
    def test_silence_propagates_to_a_stop(self):
        ...
        while time.monotonic() < deadline:
            self._spin(0.5, publish_obstacles=True)
        ...
        self._spin(6.0, publish_obstacles=False)

        # ---- 断言 1：规划器在 obstacle_timeout + 余量内报 ERROR「没有更新」 -----
        errors = [p for p in self.planning_samples
                  if p[0] > t_silence_s and '没有更新' in p[2]]
        self.assertGreater(
            len(errors), 0,
            '静默后规划器从未报「障碍物列表没有更新」—— 过期检查没生效。'
            f'静默后的规划消息：{sorted({p[2][:30] for p in self.planning_samples if p[0] > t_silence_s})}')
        planning_latency_s = errors[0][0] - t_silence_s
        self.assertLess(
            planning_latency_s, OBSTACLE_TIMEOUT_S + 0.5,
            f'规划器过期判定太慢：{planning_latency_s:.3f} s（阈值 {OBSTACLE_TIMEOUT_S} + 余量 0.5）')
        ...
        self.assertLess(
            control_latency_s, OBSTACLE_TIMEOUT_S + TRAJECTORY_TIMEOUT_S + 0.7,
            f'整条链路降级太慢：静默后 {control_latency_s:.3f} s 才 TRAJECTORY_STALE'
            f'（两段阈值 {OBSTACLE_TIMEOUT_S}+{TRAJECTORY_TIMEOUT_S} + 余量 0.7）')
        self.assertLess(
            abs(self.control_samples[-1][2]), 0.05,
            f'静默后车没有停住：末拍速度 {self.control_samples[-1][2]:.3f} m/s（静默时 {v_at_silence:.3f}）')
        print(
            f'\n  [P9-S5b] 感知静默于 t={t_silence_s:.2f}（v={v_at_silence:.2f} m/s）：'
