...
⚠️ **但它不是稳定通过的** —— 2026-08-11 同配置第三轮实测 **52 m 失败**，
法向散布低到 8.23e-07（只看得见地面），而失锁恢复在那种情况下**无能为力**
（粗网格同样退化，`coarse.degenerate` 成立 → 恢复实测 0 次触发）。
**验收路线上仍存在结构稀疏路段**，是 P4-1 决策一那件事的残余。见 todo.md。
...
0.5 m 量级的锁错要靠**标定过协方差的严格卡方**，那一条列为 **P8 前置条件**。见 localization.md §10.6。
