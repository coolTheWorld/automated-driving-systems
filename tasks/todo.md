...
**当前位置**：**P2 控制 —— S4 做到 4.6，🚩 停在 CP-P2-B：8 项判据 4 项未达标。**
根因已用独立测量定位（见 P2-S4 那节），**下一步需要决策，不要直接开工**。

三个待决策项：

1. **转向执行机构滞后 1.2 s** —— 是改 SDF 让它符合 `vehicle_params.yaml`
   已经声明的 0.5 rad/s（动**车辆模型**，两个仿真环境共用的单一来源，
   影响 P0b 对齐），还是调低 `k_e` 迁就它（把 Gazebo 特有的产物烙进增益，
   正是 SPEC §4.1 警告的做法）？**我的建议是前者**，理由见 control.md §3.9。
2. **速度环缺加速度前馈** —— 补 `a_ff = v·dv/ds` 是结构性补全，不是调参；
   但它要改 `lib/speed_controller` 或 `speed_profile` 的接口。
3. **4.4 只实测触发了 NO_PATH**，另三种降级路径已实现但未逐条触发验证；
...
| 项 | 判据 | 实测 | 结果 |
|---|---|---|---|
| 最大横向误差 | < 0.30 m | **0.801 m** | ❌ |
| 横向误差 RMS | < 0.10 m | **0.252 m** | ❌ |
| 稳态速度误差 | < 0.20 m/s | **0.127 m/s** | ✅ |
| 最大横向加速度 | < 2.0 m/s² | **2.113 m/s²** | ❌ |
| 转向速率 | ≤ 0.500 rad/s | **0.5000000005** | ✅（超出 0.9 ppb，浮点残渣） |
| 终点停车位置误差 | < 1.0 m | **4.254 m** | ❌ |
| 终点残余速度 | < 0.05 m/s | **0.006 m/s** | ✅ |
| 单拍控制耗时 | < 10 ms | **3.0 ms** | ✅ |

**判据一条未放宽。** 三个根因都已用独立测量定位：

### 🔬 根因一：Gazebo 的转向执行机构有 1.2 s 滞后（前四项里的三项）
