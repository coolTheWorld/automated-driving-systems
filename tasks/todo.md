...
> 　　**当前阶段：P3 运动规划（Frenet 采样）—— S1 ✓、S2 ✓（CP-P3-A 达成），下一步 S3**
...
**当前位置**：**P3 的 S1、S2 均已完成，CP-P3-A 达成。下一步 S3（横向 lattice 采样
+ 碰撞检测 + 代价函数）。速度规划归属已拍板：移到 `ads_planning`。**
...
| S3 | 横向 lattice 采样 + 碰撞检测 + 代价函数 | **← 下一步** |
| S4 | 速度规划迁移 + 障碍物触发减速 | 待办（已拍板：移到 `ads_planning`） |
