...
**当前位置**：**P2 完成 28/28（CP-P2-A / CP-P2-B 均达成）。
用户已选定 P0b 方案 B（最小对齐验证）—— 本地准备就绪，卡在"需要一台云 GPU 机器"。**

### P0b-B 的状态：本地能做的都做完了，剩下的必须上机

| # | 事项 | 状态 |
|---|---|---|
| B.0 | **先验本机能不能跑**（省掉租机器的钱） | ✅ **不能** —— 只有 llvmpipe 软件 Vulkan，见下 |
| B.1 | 对齐 + 测量脚本 `scripts/carla_align_vehicle.py` | ✅ 写好，`--dry-run` 通过 |
| B.2 | 上机手册 `docs/p0b_minimal_alignment.md` | ✅ |
| B.3 | Gazebo 侧基线 | ✅ τ = **0.294 s**，达成率 **100.7%** |
| B.4 | **起云 GPU 机器 → 验 Vulkan → 装 CARLA → 跑脚本** | ⛔ **需要你的账号和钱，我做不了** |

**⚠️ B.0 是这次最有价值的一条，而且免费**：本机**没有硬件 Vulkan**，
只有 `llvmpipe`（CPU 软件光栅化）。`verify_gpu.sh` 全过说明的是 **OpenGL**
（Mesa d3d12 → `/dev/dxg`），**Vulkan 是另一条路**，WSL 下要靠 `dzn` 而它没装。
CARLA 强制要求 Vulkan → **本机跑不了** → 这正是 SPEC §4.1 把环境 B 放云端的
直接原因，现在有实测依据而不是假设。零安装验法见 `CLAUDE.md` 环境陷阱表。

**⚠️ 脚本从未在真实 CARLA 上跑过。** `--dry-run` 只覆盖参数映射与单位换算
（最容易错、也最不该在计费机器上调试的部分）；连接、spawn、同步模式、
`get_physics_control()` 的字段名只能上机验。**第一次上机请留排错时间。**

**接下来你可以**：① 提供一台云 GPU 机器（或让我知道怎么连），我把 B.4 跑完；
② 或者先进 **P3 规划（Frenet 采样）**，B.4 挂着 —— 准备工作已经做完，
随时上机随时能跑，不会因为拖延而作废。
