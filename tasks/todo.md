...
> **当前阶段：P2 控制 21/28**（S1 ✓、S2 ✓、S3 ✓、S4 4/6 —— CP-P2-A 达成，
> **CP-P2-B 未达标，停在检查点**）
...
**当前位置**：**P2 控制 —— S4 做到 4.6，🚩 停在 CP-P2-B：8 项判据 2 项未达标**
（初测 4 项；2026-08-03 用户拍板"改 SDF"后横向两项已达标）。

**已修**：转向执行机构滞后 —— `vehicle_params.yaml` 新增
`actuator.steer_response_time_s = 0.050`，`gen_vehicle_model.py` 推导出
`<steer_p_gain> = 20`。实测 63% 上升时间 **1.198 → 0.294 s**；
最大横向误差 **0.801 → 0.0633 m**、RMS **0.252 → 0.0159 m**。
回归：`verify_sim.sh` 通过（RTF 0.990）、`verify_teleop.sh` 4/4、两个 `--check` 同步。

**剩余两项是同一个根因，仍需决策**：

1. **速度环缺加速度前馈**（`最大横向加速度 2.113` + `终点冲过 4.26 m`）。
   一阶系统跟踪斜坡的稳态误差 = 斜率/`K_p` = 3.0 m/s。补
   `a = v·dv/ds + K_p·(v_ref − v)` 是**结构性补全不是调参**，
   但要动 `lib/speed_profile`（需要暴露剖面的斜率）或 `speed_controller` 的接口。
   > ⚠️ 反直觉的一点：修好转向后横向误差降了 12.7 倍，**横向加速度一个数都没变** ——
   > 因为它是超速造成的，不是跟踪误差造成的。**别去调横向增益。**
2. **4.4 只实测触发了 NO_PATH**，另三种降级路径已实现但未逐条触发验证；
   **4.5 的 RViz 可视化还没肉眼看过**（全程 headless 跑的）。
...
docker compose exec dev bash -c '
  ...
  cd /workspace
  colcon test-result --all                 # 应为 395 tests, 0 errors, 0 failures（P2-S4 后）
...
| 4.4 | 安全降级：无路径 / 路径过期 / TF 丢失 / 偏离过远 → **减速停车并各自打日志** | 四种各触发一次，日志说得出是哪一种 | ⚠️ 已实现 4 种 + 到达终点，**只实测触发了 NO_PATH** |
| 4.5 | RViz：最近点、前轴投影、横向误差、目标 vs 实测速度 | 肉眼可读 | ⚠️ 已发 `/control/debug`，**未肉眼确认**（headless 跑的） |
| 4.6 | **闭环实测 + 数据采集**【CP-P2-B】 | 见下表 | ❌ **8 项中 4 项未达标**，根因已定位 |
...
| 项 | 判据 | 初测 08-03 | **改 SDF 后** | 结果 |
|---|---|---|---|---|
| 最大横向误差 | < 0.30 m | 0.801 | **0.0633** | ✅ |
| 横向误差 RMS | < 0.10 m | 0.252 | **0.0159** | ✅ |
| 稳态速度误差 | < 0.20 m/s | 0.127 | **0.1273** | ✅ |
| 最大横向加速度 | < 2.0 m/s² | 2.113 | **2.1133** | ❌ |
| 转向速率 | ≤ 0.500 rad/s | 0.5000000005 | **0.5000** | ✅ |
| 终点停车位置误差 | < 1.0 m | 4.254 | **4.2589** | ❌ |
| 终点残余速度 | < 0.05 m/s | 0.006 | **0.0059** | ✅ |
| 单拍控制耗时 | < 10 ms | 3.0 | **5.0** | ✅ |

**改 SDF 之后 8 项里 6 项达标（原来 4 项）。** 只改了一个参数
（`actuator.steer_response_time_s`），横向误差降了 **12.7 倍**、RMS 降了 **15.8 倍**。
剩下两项**是同一个根因**（见下），改横向增益动不了它们。
...
S4 ████__  4/6      S5 _____  0/5                        总计 21/28
                    （4.4 部分、4.6 未达标 —— CP-P2-B 停在这里）
```
