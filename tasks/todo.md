...
> 状态：**P0a 完成 29/29**、**P1 完成 28/28**（CP-P1-A / CP-P1-B 达成）　|　**当前阶段：P2 控制**
> 更新：2026-08-01　|　技术栈：**Ubuntu 24.04 + ROS 2 Jazzy + Gazebo Harmonic**（官方组合）
>
...
**当前位置**：**P2 控制 —— S1 完成 6/6，下一步是 S2 横向 Stanley（到 CP-P2-A 停下来汇报）。**
计划见 [plan.md 第三部分](./plan.md#第三部分p2-控制stanley--pid)。

S1 产出：`docs/modules/control.md`（8 节推导）+ `ads_control` 包 + `lib/path_tracking`
（16 个 L1 用例、0 ms、6 处故障注入全部被预期用例抓住）。
全量回归 **285 tests / 0 failures**（基线 248）。
自检时抓到并修掉一处真缺陷（NaN 绕过点距校验，详见 P2-S1 实测那节）。

**S2 开工前先读 `docs/modules/control.md` §3**，尤其 §3.4 和 §3.5 ——
那两节推翻了「Stanley 在定曲率上有固有稳态误差」和「顺手加个曲率前馈更好」
两个几乎一定会犯的直觉。
...
docker compose exec dev bash -c '
  ...
  cd /workspace
  colcon test-result --all                 # 应为 285 tests, 0 errors, 0 failures（P2-S1 后）
...
| # | 任务 | 验收标准 | 状态 |
|---|------|---------|:---:|
| 2.1 | **前轴换算**：`base_link` 沿车头前移 `wheelbase_m`(2.700) | 车头朝任意角度时前轴位置正确 | ☐ |
| 2.2 | 控制律 `δ = ψ − atan(k_e·e/(k_soft+v))`（**减号**，因本项目 `e` 左正） | 与手算一致 | ☐ |
| 2.3 | 转角限幅 0.600 rad + **转向速率**限幅 0.500 rad/s | 任意输入下两者都成立 | ☐ |
| 2.4 | 运动学自行车模型（**测试夹具**，非产品代码） | 定转角 → 半径 `L/tan(δ)` 的圆，与解析解一致 | ☐ |
| 2.5 | **闭环收敛测试**【CP-P2-A】 | 见下三条 | ☐ |
| 2.6 | **定曲率稳态用例**：守住「不要加曲率前馈」 | R=8 稳态 \|e\| < 0.05 m（加了前馈必红，理论值 1.421 m） | ☐ |
...
| 场景 | 判据 | 解析值 | 仿真实测 |
|---|---|---|---|
| 直线，初始 \|e\|=1.0 m，v=5.556 | 5 s 内 \|e\| < 0.05 m，超调 < 5% | 3.00 s | **3.42 s**，超调 **0.00%** |
| 直线，初始 ψ=30°，e=0 | 5 s 内 \|e\| < 0.05 m | — | **2.64 s** |
| R=8 圆弧，v=3.464 | **稳态** \|e\| < 0.05 m | **0** | **0.0162 m** |
| 直路 → R=8 圆弧 | **入弯峰值** \|e\| < 0.10 m | — | **0.0166 m** |
| 定曲率 + **前馈**（反例，2.6） | 必须 ≫ 判据，证明用例有鉴别力 | 1.421 m | **1.443 m** |
...
>
> **2.3 的速率限幅必须进闭环 L1 测试**：0.500 rad/s 意味着零到满舵要 **1.2 s**，
> 这是一个不小的相位滞后，可能把无速率限制下稳定的增益变成震荡的。
> **在 L1 里发现比在 Gazebo 里发现便宜两个数量级。**
>
...
```
S1 ██████  6/6 ✓    S2 ______  0/6      S3 _____  0/5
S4 ______  0/6      S5 _____  0/5                        总计  6/28
```
