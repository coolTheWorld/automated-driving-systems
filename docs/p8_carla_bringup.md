...
| 4 | throttle→加速度强非线性 | 怠速顶不动风阻、0.54 bias 断崖、20 FPS 粗粒度；中速段增益偏大（**根源见 #8：零油门发动机阻尼 2.0 vs 全油门 0.15 + 扭矩曲线峰在 1 挡 4.8 m/s**） | bias+k 映射 + 怠速死区 + **驻车闩锁** + 上升速率限幅。**弯道超调残留**（目标 3.9 实测 4.6）→ 差异 #8 | 标定 + junction tap；窗口 6 二维表 |
| 5 | brake 低开度即凶 | brake 0.2 → −3.31 m/s²（Gazebo 线性） | 0.185/m/s² 标定；停车翻转期的微冲-停已由孤儿定性消解 | 窗口 1 标定 |
| 6 | 路外语义 | Gazebo 草肩（出线无代价）vs CARLA 生成世界路外即虚空（坠落 = 引擎 Segfault） | **additional_width 6.0 硬化路肩**（0.6 时弯道外漂亲墙被物理逮捕，控制命令 +1.5 实测 −4.6；P9-S2 退到 6.0）。护栏墙 wall_height 1.0 → **0（P9 窗口 4）**：墙在弯角是 L 形连通簇，OBB 52×15 m 空腹框中心落在车道 → 结构物 → 感知层场景车停在东南角前 20 m、353 帧车道内虚警。Gazebo 同处是草地，撤墙后两环境感知输入同构；6 m 裙板留作第一道容错，出界 6 m 以上仍会坠虚空崩引擎（行为/跟踪判据全在路内） | junction tap 铁证；P9 窗口 4 round 2/3 |
| 7 | NPC 道具语义 | Gazebo 道具无碰撞无重力；CARLA spawn 做碰撞检查、仿真钟粒度 0.05 限定时器 20 Hz；P9-S2 起车辆走 apply_control 物理闭环（重力开） | 高空 spawn+瞬移归位（行人）；车辆共用首航点的车队 → **天上停车场**（关重力挂着等出发相位，第一条非零 cmd 时落到首航点；P9 窗口 5 junction）；dt 用实际仿真流逝；pose_gt 补 twist；道具蓝图 seat.leon（贴 ODD 几何，见 npc_kinematics） | 行为三连 14/14；窗口 5 6/6 |
| 8 | **跟踪质量（S01 四条，差异入表）** | 弯道油门映射增益误差 → 冲宽-急刹循环：max_lateral 0.84（判据 0.3）/ rms 0.17（0.1）/ Δv 0.42（0.2）/ a_lat 3.04（2.0）。安全类判据（碰撞/间距/行为正确性 14/14）全绿不受影响。**机理坐实（P9-S5d，窗口 6）**：PhysX 发动机净扭矩 = th·T(rpm) − [d0 − (d0−d1)·th]·ω —— 偏置就是「顶住零油门阻尼 d0=2.0」、随速度 0.48–0.55；增益随扭矩曲线走（峰 1700 rpm ≈ 1 挡 4.8 m/s），4–5 m/s 箱比 2–3 箱高 78%（`scripts/carla_throttle_map_probe.py` 二维表）。对齐实验（d0=d1、扭矩恒 350）静态图变直，首轮闭环只赢 Δv（0.42→0.25），低速蠕动死区把车停在目标前 3.96 m | **入表放行**（用户拍板先例）；根治 = 独立战役（蠕动偏置 / 制动-滑行配平 / PI 复调，估 1–2 窗口），机制已固化 `ego.align_drivetrain`（默认关）—— **待拍板做不做** | 窗口 2 S01 ×2；窗口 6 S01 ×2 + 两张表 |
