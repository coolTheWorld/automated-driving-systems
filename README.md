...
当前阶段：**P5 感知已完成（Gazebo 半），全栈复检与修复收口**（2026-08-12）。
车关掉一切真值后：用传感器自主定位（横向误差 0.094–0.111 m，判据 0.30，
六轮全过），用真感知看世界（检测率 100%，近边距离误差 ~0.12 m，判据 0.5，
行人被车挡住 1.8 s 再出现 ID 不变），规划器吃真感知仍能绕过贴边障碍物
（侧向间距 0.532 m，判据 > 0.5）、对几何无解的障碍物停住并报「不可行」。

此前已完成：P0a 本地环境、P1 地图与路由、P2 控制（CP-P2-B 8/8）、
P3 规划（CP-P3-B 三场景 + plan 全表 9/9）、P4 定位（ESKF + NDT）、
P0b 方案 B（在云 GPU 上实测出 CARLA 与 Gazebo 的转向执行机构差异）。
2026-08-12 做过一次全栈对抗性复检（37 个审查/验证子代理）：
修复 3 high + 11 medium 缺陷（含失锁恢复整条死代码、看门狗可被 NaN 流
喂死、判据仪器四处量错对象），过程与教训见 tasks/todo.md。
