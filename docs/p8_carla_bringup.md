...
| 13 | **雷达自反射（车顶）** | c3 车顶顶点 1.571 m，雷达 mount_z 1.6 → 相差 2.9 cm，**60% 射线打自己车顶**（每 tick 11k/18k），后左扇区连 −5° 通道都被挡；CARLA 无 `range_min` 近裁剪，Gazebo 的透顶技巧不存在。裸测：1.9 剩 4%，2.2 归零 | **保留，待拍板**：改挂高要动 vehicle_params 单一来源（Gazebo SDF/URDF 一起变、P5 Gazebo 基线重跑）。近场（车过身、后侧）检测受它影响最大 —— round 4 车 3-10 m 速度误差 p95 1.64 的候选原因之一 | `p9_lidar_probe.py --lidar-z` |
| 14 | **远距聚类断裂（32 线 × 0.5 m 容差）** | ≥25 m 处相邻通道在竖直面上间距 ≈0.49 m ≈ 聚类容差 0.5：Gazebo 盒子车平面正对面勉强连通，micra 保险杠/引擎盖前后错开就断成 2 簇（1.2–1.7 × 0.1–1.0 的条），跟踪器在两条之间摆 → ID 切换、近边 ±1 m、速度 p95 3.08（20-25 m） | **未处置**（P9-S3 候选：按距离自适应容差 / 2D 投影聚类，属共享感知代码，须带 Gazebo 回归） | round 4 时间线 t=17-19.6 |
