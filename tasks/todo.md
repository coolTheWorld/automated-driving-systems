...
> **当前阶段：P2 控制 22/28**（S1 ✓、S2 ✓、S3 ✓、S4 5/6 ——
...
**S4 剩下的两条尾巴（不阻塞 S5，但别忘了）**：

1. **4.4 只实测触发了 NO_PATH**。另三种在当前拓扑下**不可达**（原因见 P2-S4 那节），
   不是"没去试"。顺带发现一个真缺口：**仿真钟停走时控制器会冻住而不是降级**。
2. **4.5 的 RViz 可视化从没肉眼看过**（全程 headless）。这条只能人来看。
...
| 4.5 | RViz：最近点、前轴投影、横向误差、目标 vs 实测速度 | 肉眼可读 | ⚠️ 已发 `/control/debug`，**未肉眼确认**（全程 headless 跑的） |
...
S4 █████_  5/6      S5 _____  0/5                        总计 22/28
                    （4.4 只触发了 1/4 种降级，4.5 未肉眼确认）
```
