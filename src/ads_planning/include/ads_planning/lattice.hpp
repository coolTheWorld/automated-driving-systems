...
struct LatticeParams
{
  ...
  /// 非新机动候选（|d_T − d₀| ≤ 半网格步：延续既成轨迹）的保底间距，单位 m。
  ///
  /// **两级准入（P8-S6，用户拍板）**：margin 是**选择新轨迹**的设计余量；
  /// 执行误差把车压到 margin 之下是**既成事实**，只要不破 SPEC §8 的安全线
  /// （0.5，本值的出处）就允许把当前轨迹走完 —— 不允许的是**新机动**主动
  /// 把间距降到 margin 以下。
  ///
  /// 不分层的死锁（CARLA 实测，margin=0.7）：规划 d=0.8 过锥，执行滑落到
  /// 实际间距 0.697 < 0.70 ⟹ 从该位姿看**全部候选**（含延续候选）都
  /// 不达 margin ⟹ 27/27 全灭 ⟹ 急刹死锁在锥旁 103 s。通解公式：
  /// 延续候选间距 ≈ margin + 网格富余 − 执行误差，执行误差 > 富余时必死。
  ///
  /// 约束：0 < floor ≤ margin（构造时校验）。floor == margin 即退化回
  /// 单级准入 —— 那正是死锁形态，test_lattice 有用例钉着。
  double safety_floor_m{0.0};

  /// 候选自身峰值曲率的准入上限，单位 1/m（P8-S2d）。
  ///
  /// 物理上界是车辆的运动学能力 `tan(max_steer)/wheelbase`（本项目 0.2534，
  /// R_min ≈ 3.95 m）；调用方按「能力 × 转向余量系数」填（0.8 ⟹ 0.203），
  /// 给 Stanley 的跟踪修正留两成转向权限 —— 规划把转向用满，控制器就没有
  /// 余量纠偏了。
  ///
  /// 为什么需要这道门（P7-S4 junction 实测）：末端短跑道上强行收敛的五次式
  /// 峰值曲率可达 2.9（R = 0.35 m），**车根本开不出来** —— 这种候选此前
  /// 要么被旁边障碍物挡下（全候选淘汰 → 车在离 goal 1.16 m 处停保持），
  /// 要么被选中后让车蠕行。按"开不开得出来"直接淘汰才是对的语义。
  ///
  /// ⚠️ 峰值曲率必须**密采样五次式**求（见 lattice.cpp 的 peak_curvature）：
  ///    输出点距 0.5 m 在短机动上恰好采在 |d″| 的零点（中点是拐点），
  ///    峰值藏在采样点之间 —— 速度剖面与代价此前都因此看不见它。
  ///
  /// 调大 → 放进不可跟踪的候选；调小 → 弯道上的正常绕行也被淘汰
  ///        （参考线偏移曲率 κ/(1−dκ) 本项目最大约 0.185，不能低于它）。
...
struct LatticeResult
{
  ...
  /// 因**运动学不可跟踪**（峰值曲率超限）被淘汰的候选数（P8-S2d）。
  /// 与 blocked_count 分开 —— 「撞」和「开不出来」在 diagnostics 里
  /// 必须分得开，排查方向完全不同。
