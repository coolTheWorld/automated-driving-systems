...
//
//  ## ⚠️ 竖向容差单独给（P9-S3a，2026-08-15）—— 线间距顶的是**竖向**，不是水平
//
//  上面那笔账的两侧其实不在同一个方向上：**顶着下界的是竖向线间距**
//  （0.0197·d），**要守住的 1.0 m 分辨力是水平的**（两个目标并排）。
//  一个各向同性的 tolerance 把两件事绑在一起，只能在 25 m 上折衷。
//  拆开：水平仍 0.5，竖向 `vertical_tolerance_m` = 1.0 —— 实现是聚类前把 z 乘以
//  tolerance/vertical_tolerance（椭球邻域，体素哈希与 BFS 一个字不改），效果：
//
//      · 纯竖向的相邻两线：连通极限从 25.4 m 推到 50.8 m（≥ 量程 50）
//      · 相邻两线之间还差一个**进深台阶** d（保险杠 → 格栅 → 引擎盖，
//        真车正面不是一块平板）：25 m 处可连的台阶从 0.10 m 放宽到 0.44 m
//        （√(d² + (0.493·0.5)²) ≤ 0.5）—— CARLA micra 正对驶来 23–33 m 时
//        断成两簇、跟踪器在两条之间摆（ID 切换 4–6、近边 ±1 m）的病根；
//        Gazebo 的平板车 25–30 m 档检测率 90–92% 也是这一条。
//      · 水平方向分毫不动：并排 1.0 m 的两个目标照旧分开。
//
//  代价：竖直方向 1.0 m 内叠放的东西会并成一簇（园区 ODD 里没有悬空物；
//  地面分割残留的路沿点若贴着目标脚下 ≤1 m，会把框往下拉一点）。
//  取 1.0 而不是更大：再大只是把「叠放合并」的半径白白放宽，竖向连通
//  在 1.0 时已经覆盖整个量程。
//
//  ## ⚠️ 质心与包围盒中心**不是一回事**
//
...
struct EuclideanClusterParams
{
  ...
  /// 竖向邻域半径，m（P9-S3a）。见文件头「竖向容差单独给」那一节。
  ///
  /// 实现：聚类前 z 乘以 tolerance_m / vertical_tolerance_m。等于 tolerance_m 时
  /// 退化为各向同性（P5 验收时的行为）。
  /// 调大 → 竖直叠放的东西更容易并簇；调小到 tolerance_m 以下没有物理理由
  /// （线间距在竖向，只会把目标按扫描线拆开）。
