...
struct SpeedProfileParams
{
  ...
  /// 过弯的横向加速度上限，m/s²。来自 `planning_params.yaml` 的
  /// `speed.max_lateral_accel_mps2`（**1.15**，P3-S5 从 1.5 降下来）。
  ///
  /// 调大 → 过弯快（2.0 时 R=8 处限速从 3.033 升到 4.000）；调小 → 全程慢。
  ///
  /// ⚠️ **这是规划侧的限值，不等于车实际受的横向加速度。** 跟踪修正叠在
  ///    路径曲率之上，CP-P3-B 实测超调 55–66%：限 1.5 实测 2.33，限 1.15
  ///    才落到判据 2.0 以内。「舒适区 1.5–2.0」说的是**实测值**，直接填进来
  ///    实测就到 3.3。整条依据见 `planning_params.yaml` 的注释。
  ///
  /// 乘用车物理极限 8–9 —— 但**运动学模型不体现轮胎打滑**，
  ...
  /// 逐点速度上限（P7-S2 新增，可选），m/s，与弧长数组逐一对应；**空 = 不加**。
  ///
  /// 这是行为层（`lib/behavior`）把「让行/慢行」注入剖面的机械通道：
  /// 上限在曲率限速**之后**、前后向扫描**之前**取 min，扫描随后把任何突降
  /// 自动展开成可行的减速/加速斜坡 —— 与停车剖面用 terminal=0 是同一个原理。
  ///
  /// ⚠️ **限速的"决策"不在这里**（本文件头的红线：不要往剖面里加避障）。
  ///    这里只做机械展开；哪里该限、限多少，由行为层给出、在 plan() 合成。
  ///    空数组时**逐点严格等于**现剖面（CP-P7-A ④ 的回归判据钉着这条）。
  ///
  /// 单点填 0 合法：等价于"在该点停住"，扫描会把减速段铺到它前面。
