...
| # | 任务 | 验收标准 | 状态 |
|---|------|---------|:---:|
| 4.1 | `control_node`：订阅 `/route/path` + TF + `/odom`，发 `/vehicle_cmd` @50 Hz | 话题接得上 | ☐ |
| 4.2 | `config/control_params.yaml` + launch 装配（车辆限值仍从 `vehicle_params.yaml` 来） | 无任何硬编码限值 | ☐ |
| 4.3 | **修 `/route/path` 的 QoS 为 `transient_local`** | 两种启动顺序各跑一次（见下） | ☐ |
| 4.4 | 安全降级：无路径 / 路径过期 / TF 丢失 / 偏离过远 → **减速停车并各自打日志** | 四种各触发一次，日志说得出是哪一种 | ☐ |
| 4.5 | RViz：最近点、前轴投影、横向误差、目标 vs 实测速度 | 肉眼可读 | ☐ |
| 4.6 | **闭环实测 + 数据采集**【CP-P2-B】 | 见下表 | ☐ |
...
| # | 任务 | 验收标准 | 状态 |
|---|------|---------|:---:|
| 5.1 | `control.md` 补实测、参数表（含**调大调小的后果**）、已知边界 | SPEC §11 | ☐ |
| 5.2 | **假车闭环进 CI**（自行车模型包成 ROS 节点，发 `/odom`+TF+`/clock`） | 不需要 GPU、秒级、结果确定 | ☐ |
| 5.3 | SPEC §3.2 加纵向内环脚注；§10 的 P2 行按实测更新 | 与事实一致 | ☐ |
| 5.4 | `CLAUDE.md` 同步新包/新命令/新陷阱 | 无过时说法 | ☐ |
| 5.5 | **P0b（CARLA）重新摆上桌面**：给出接入成本与不接的代价，交用户拍板 | 有可决策的材料 | ☐ |
