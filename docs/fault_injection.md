...
| 5 | 感知从未到达（`expect_perception=true` 而话题空） | 规划指名报错「从未到达」不发轨迹；控制恒 NO_PATH | **`test_perception_silence.py` 阶段 0（2026-08-16 复审补断言）**：不发任何障碍物直接给目标 3 s ⟹ 规划「从未到达」有拍、控制无 TRACKING、v<0.05；随后流一到车动起来（恢复）。此前只是「顺带被走到」没有断言 | ✅ |
...
| 10 | 两套仿真并存（`/clock` 双源，dt ≤ 0 或巨 dt） | 感知丢帧 + 跟踪器按首帧重来；脚本层残留守卫拒跑 | perception_node dt 守卫（先守卫后更新 last_stamp，2026-08-12 修）；`run_all_scenarios.sh`/`l3c_*_round.sh` 残留守卫 | ✅。⚠️ 2026-08-16 复审：`run_all_scenarios.sh` / `l3g_p5_round.sh` 的残留守卫此前是**空操作**（awk 打整行、grep -x 永远匹配不上），已修并实测（起一个 comm=control_node 的进程：旧 0 → 新 1） |
| 11 | 定位失锁 / 雷达断流 / GNSS 断流 | NDT `max_innovation` 安全阀；连续 ≥10 帧超门强制粗网格重锚；**NDT 1.0 s 无成功帧降出 NDT_AIDED；GNSS 2.0 s 超时进 DEAD_RECKONING；降级不冻结（map→odom 继续发）** | L3-G 定位闭环 ③c（`ndt_chi2_exceed` 计数）+ 注入实测 32 次重锚；**`test_localization_sensor_timeout.py`（P9-S5b 新增，domain 54）**：假传感器 `fault.lidar_stop_after_s=12`/`fault.gnss_stop_after_s=17` ⟹ 14.0 s 首个 GNSS_ONLY、19.0 s 首个 DEAD_RECKONING、降级期间位姿 400+500 条；注入 NDT 超时 1e9（复原 2026-08-12 前的锁存缺陷）红 | ✅。⚠️ 只测了**降级**：雷达/GNSS 恢复后重新锁回 NDT_AIDED 无自动化用例（假传感器无 resume 开关）—— 已知缺口 |
| 12 | 真值道具飞天 / 埋地 / 倾倒 | 记录器判「本次运行无效」拒绝打分 | `record_perception_run.py` 刺激物校验（现在读**物理** z）；`p9_lidar_probe` 落定高度 | ✅（评测层） |
| 13 | TF 缺失（`map→base_link` 从未到达） | 感知跳帧 warn；规划不发；控制 NO_PATH；TF 到位后重发目标即恢复 | 静态 TF 永不过期 ⟹ 只能「一开始就不发」测（陷阱表）；**`ads_control/test/test_route_faults.py`（P9-S5b 新增，domain 53；三段合成一个方法，第 ① 段）**：launch 无 map→odom ⟹ 4 s 恒 NO_PATH v=0；测试补发静态 TF + 重发目标 ⟹ TRACKING；注入「launch 里加静态 TF」红。L3-G 定位闭环另机械查 `/tf_static` 上不许有 `map→odom` | ✅ |
...
| 15 | 目标点无路由 | map_node WARN + 空 Path 清屏；规划忽略 <2 点、保留上一条路线；控制不动 | `test_routing`（L1 无路可达）；**`test_route_faults.py`（第 ③ 段）**：(500,500) ⟹ 4 s 恒 GOAL_REACHED v=0；注入换成路上合法目标红。⚠️ 第一次注入用的 (51.75,20) 在草地上、本身也无路由，注入后照样绿 —— 注入没红先查注入是不是有效刺激 | ✅ |
| 16 | 畸形 PointCloud2（`point_step=0` / data 长度不是整数倍 / 缺 z 字段） | 结构校验丢弃并计数 `dropped_malformed_clouds`，节点不挂死不崩；下一帧正常帧照常输出 | perception_node `CloudLayoutOk` + lidar_preprocessor `cloud_layout_ok`（同一份检查，两处）；**`test_closed_loop_perception` #16 段**：三种畸形各一帧 ⟹ 计数 3、随后 5 帧非空输出。注入把校验关掉 ⟹ 节点 **exit −11**（sensor_msgs 迭代器按 point_step 步进、指针不等判 end —— point_step=0 死循环、长度不整越界读）| ✅（2026-08-16 安全复审 Medium） |
...
表里现在 16 行全是 ✅（#8 的「滤净」那一半仍要 GPU，L1 那一半进 CI；#10 的守卫在脚本层）。
