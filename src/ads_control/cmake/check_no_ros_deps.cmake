...
#  测试一慢，人就不跑了，测试就白写了。
#
#  但这条约束的**落点很脆**：本包的 CMakeLists 里同时有 libads_control.so
#  和 control_node 两个 target，后者理所当然地链接了一堆 ROS。
#  哪天有人为了图方便把某个 ROS 依赖从 control_node 挪到公共位置（比如写成
#  link_libraries() 而不是 target_link_libraries()），约束就破了，
#  **而且不会有任何报错** —— 编译照过，测试照绿，只是慢了下来。
#
#  ⚠️ 本包的 CMakeLists 从 P2-S1 起就写着「P2-S4 的 verify_control.sh 会 ldd
#     这个 .so」。**那个脚本从来没被写出来过**，直到 P2-S5 逐个 grep 才发现。
#     也就是说这条约束在整个 P2 期间**只靠纪律维持** —— 而那段注释自己
#     论证的恰恰是「靠纪律维持的约束迟早会破，靠脚本维持的不会」。
#     现在补上，并且做成 CTest 而不是 shell 脚本：`colcon test` 会跑它，
#     CI 也就跟着跑了。**一个要人记得去执行的检查，等于没有检查。**
#
...
#
#  真正会被抓住的是「lib 里真的用了 ROS 符号」。实测：在 speed_profile.cpp 里
#  加一句 RCLCPP_INFO，DT_NEEDED 立刻出现 librclcpp，本检查随即失败。
#
...
if(offenders)
  ...
  message(FATAL_ERROR
    ...
    "ROS 依赖只允许出现在 node/ 的 target 上（control_node），"
